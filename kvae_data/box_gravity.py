...
if __name__ == '__main__':
    ...
    # Create data dir
    kvae_dir_data = pathlib.Path(__file__).parent.absolute().joinpath('data')
    if not kvae_dir_data.exists():
        kvae_dir_data.mkdir()

    print("Generating sequences")
    cannon = BallBox(dt=0.2, res=(32 * scale, 32 * scale), init_pos=(16 * scale, 16 * scale), init_std=8, wall=None, gravity=(0.0, -5.0))
    cannon.run(delay=None, iterations=20, sequences=6000, radius=3 * scale, angle_limits=(0, 360),
               velocity_limits=(5.0 * scale, 10.0 * scale), filepath=kvae_dir_data.joinpath('box_gravity.npz'), save='npz')
